...
import { Target, TrendingUp, AlertCircle, ShoppingCart, Key, Wrench } from 'lucide-react'
...
function MetaCard({ title, icon: Icon, data, color, darkMode }) {
  if (!data) return null;
...
export default function AbaMetas({ metas_pessoais, darkMode }) {
  if (!metas_pessoais) return null;

  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24, paddingBottom: 40 }}>
      <MetaCard title="Vendas" icon={ShoppingCart} data={metas_pessoais.vendas} color="#ec6e2a" darkMode={darkMode} />
      <MetaCard title="Locação" icon={Key} data={metas_pessoais.locacao} color="#3b82f6" darkMode={darkMode} />
      <MetaCard title="Serviços" icon={Wrench} data={metas_pessoais.servicos} color="#10b981" darkMode={darkMode} />
    </div>
  )
}
