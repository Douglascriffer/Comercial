...
export function useFinancialData() {
  ...
  const load = useCallback(async (force = false) => {
    ...
    try {
      if (!data) setLoading(true) // Não mostra spinner se for atualização em background
      const res = await fetch(`/data/dados.json?t=${Date.now()}`, { cache: 'no-store' })
...
export function useFilteredData(data, filters) {
  return useMemo(() => {
    ...
    // Adicionar o valor de Serviços manualmente para Gabriel Medeiros
    if (servicosRealizado > 0) {
      let gm = allSellers.find(s => s.name === 'Gabriel Medeiros')
      if (!gm) {
        gm = { name: 'Gabriel Medeiros', valor: 0.0, count: 0 }
        allSellers.push(gm)
      }
      gm.valor += servicosRealizado
    }
    
    // Identificar o que é "Outros Canais" em vez de "Vendedores Físicos"
    const OUTROS_CANAIS = ['Mercado Livre', 'Site', 'Retorno de golpe', 'Sem Vendedor']

    const salesTeam = allSellers
      .filter(s => !OUTROS_CANAIS.includes(s.name) && s.valor > 0)
    ...
    // O somatório de todos os vendedores inclui agora Vendas, Locação e Serviços.
    // Para não duplicar, totalRealizadoReal é simplesmente a soma de todo mundo:
    const totalRealizadoReal = salesTeam.reduce((sum, s) => sum + s.valor, 0)
                             + otherChannels.reduce((sum, s) => sum + s.valor, 0)
                             
    // O KPI de "Vendas" puras deve ser o Total menos os outros serviços:
    const vendasRealizadoReal = totalRealizadoReal - servicosRealizado - locacaoRealizado
    ...
    const historyMonths = data.byPeriod
      ...
      .map(p => ({
        ...
        'Faturamento': p.historico_faturamento || 0,
        'Meta': p.historico_meta || 0
      }))
    ...
    // 7. Agregar Metas Pessoais
    const metas_pessoais = {
      vendas: { meta_empresa: 0, meta_comercial: 0, meta_realizada: 0, restante_empresa: 0, restante_comercial: 0 },
      locacao: { meta_empresa: 0, meta_comercial: 0, meta_realizada: 0, restante_empresa: 0, restante_comercial: 0 },
      servicos: { meta_empresa: 0, meta_comercial: 0, meta_realizada: 0, restante_empresa: 0, restante_comercial: 0 }
    }

    filteredPeriods.forEach(p => {
      if (p.metas_pessoais) {
        ['vendas', 'locacao', 'servicos'].forEach(area => {
          if (p.metas_pessoais[area]) {
            metas_pessoais[area].meta_empresa += p.metas_pessoais[area].meta_empresa || 0;
            metas_pessoais[area].meta_comercial += p.metas_pessoais[area].meta_comercial || 0;
            metas_pessoais[area].meta_realizada += p.metas_pessoais[area].meta_realizada || 0;
            metas_pessoais[area].restante_empresa += p.metas_pessoais[area].restante_empresa || 0;
            metas_pessoais[area].restante_comercial += p.metas_pessoais[area].restante_comercial || 0;
          }
        })
      }
    })
